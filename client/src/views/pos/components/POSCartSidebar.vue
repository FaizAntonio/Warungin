<template>
  <aside class="glass-effect flex flex-col border-l border-slate-200 h-[40vh] lg:h-full relative shadow-2xl lg:shadow-none z-30">
    ...
    <div class="flex-1 overflow-y-auto p-5 space-y-5 custom-scrollbar">
      <div v-if="cart.length === 0" class="flex flex-col items-center justify-center h-full text-slate-500 opacity-60">
    ...
    <div class="bg-white border-t border-slate-200 p-5 pb-6 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] relative z-10">
      ...
      <div class="flex items-center justify-between gap-2 mb-4 px-1 py-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm" v-if="userRole !== 'Cashier' && userRole !== 'KITCHEN'">
        <div class="flex items-center gap-2">
